...
function MesAchats() {
  const [purchases, setPurchases] = useState<Offer[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Charger les achats de l'utilisateur connecté depuis le backend
  useEffect(() => {
    ...
    axios
      .get("http://localhost:3000/api/offers/my-purchases", config)
      .then((res) => {
      ...
      .catch((err) => {
        ...
        if (axios.isAxiosError(err) && err.response) {
          setError(err.response.data.error || "Erreur lors du chargement de vos achats");
        } else {
          setError("Erreur de connexion au serveur");
        }
        console.error("Erreur lors du fetch des achats:", err);
      });
  }, []);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <p className="mt-4 text-muted-foreground">Chargement de vos achats...</p>
        </div>
      </div>
    );
  }
                ...
                <CardFooter className="flex justify-between items-center bg-muted/50 pt-2">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant="success" className="bg-green-100 text-green-800 hover:bg-green-200">
                      <CheckCircle className="h-3 w-3 mr-1" /> Acheté
                    </Badge>
                  </div>
